    ...
    public function destroy(string $filename): JsonResponse
{
    ...
    public function store_should_fail_with_empty_filename_or_content()
{
    // Intentar almacenar un archivo con un nombre vacío
    $response = $this->postJson('/api/hello', [
        'filename' => '', // nombre de archivo vacío
        'content' => 'Contenido de prueba'
    ]);

    // Verificar que la respuesta sea un error de validación
    $response->assertStatus(422);  // Esperamos un error de validación
    $response->assertJsonValidationErrors(['filename']);  // Error por falta de 'filename'

    // Intentar almacenar un archivo con contenido vacío
    $response = $this->postJson('/api/hello', [
    ...
    // Verificar que la respuesta sea un error de validación
    $response->assertStatus(422);  // Esperamos un error de validación
    $response->assertJsonValidationErrors(['content']);  // Error por falta de 'content'
}
